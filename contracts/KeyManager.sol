 ...
 *         local key storage.
 *
 * @dev The canonical key registry lives in the {ERC734Validator} module. That module is set once,
 *      in the constructor, as an `immutable` shared across every proxy. This contract:
 *      - reads MANAGEMENT membership for {onlyManagerOrSelf} via a staticcall to the module;
 *      - forwards ERC-734 key writes (`addKey`, `addKeyWithData`, `removeKey`) to the module as
 *        self-calls;
 *      - keeps its ERC-7201 storage slot only for the `initialized` / `canInteract` flags.
 *
 ...
 *      module. They are intentionally not implemented here.
 *
...
contract KeyManager {
    ...
    /// @notice The ERC-734 key registry module ({ERC734Validator}). Baked into the implementation
    ///         bytecode, so every proxy shares it; changing it means upgrading the account.
    address internal immutable _REGISTRY_MODULE;
    ...
    /// @param registryModule The {ERC734Validator} that holds the key registry. Reverts on zero.
    constructor(address registryModule) {
        require(registryModule != address(0), Errors.ZeroAddress());
        _REGISTRY_MODULE = registryModule;
    }
     ...
     *      Forwards to the module, which derives the keyHash from `_signerData`.
     */
    ...
    /// @dev The ERC-734 registry module.
    function _registryModule() internal view returns (address) {
        return _REGISTRY_MODULE;
    }

    /// @dev MANAGEMENT / purpose read for the account itself, backed by the registry module.
