...
contract ProxyTest is OnchainIDSetup {
    function test_revertBecauseImplementationIsZeroAddress() public {
        vm.expectRevert(
            abi.encode(ERC1967Utils.ERC1967InvalidBeacon.selector, address(0))
        );
    ...
    function test_implementationAuthority_shouldReturnCorrectAddress() public {
        ...
        address storedBeacon = address(
            uint160(uint256(vm.load(address(proxy), beaconSlot)))
        );
        assertEq(
            storedBeacon,
            address(authority),
            "Should store the correct authority address as beacon"
        );
    }
